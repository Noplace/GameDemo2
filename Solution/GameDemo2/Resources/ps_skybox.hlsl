...
{ 
  float3 sunDirection = normalize(float3(-(0.5)*2., -1, (0.5)*2.));
	float3 resolution = float3(1,1,1);
    // General parameter setup
  //input.uv.x = 1 - input.uv.x;
	float3 vPos = 2.0*input.Tex/resolution - 1.0; 					// map vPos to -1..1
  ...
  cam.lookAt = float3(0,0,3);	;//							// The point the camera is looking at
	cam.position = float3(0, 5, 0);	;//				// The position of the camera
	cam.up = float3(0,0,1);;//float3(0,0,1);									// The up vector, change to make the camera roll, in world space
...
float4 main2( SkyboxVS_Output Input ) : SV_TARGET
{
