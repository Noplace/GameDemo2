Texture1D permTexture : register(t1);
Texture1D gradTexture : register(t2);
/*Perlin Noise*/
...

{
  ...
  return permutation[p * 256] / 255.0;//float4(1,1,1,1);
...
float perm(float x) {
 ...
 // return tex1D(permSampler, x / 256.0) * 256;
  ...
  //return GeneratePermTexture(x / 256.0f) * 256.0f;
}
